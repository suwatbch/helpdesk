<?php
    ...
    class helpdesk_resolve extends model {
        ...
        public function search_resolve_priority($cratiria = null, $page = 1){
            global $page_size;
            ...
            $field = "rp.*, c.cus_company_name, p1.prd_tier_name as prd_name_class1, p2.prd_tier_name as prd_name_class2,
                        ...
                        o3.opr_tier_name as opr_name_class3";
            # table
            $table = "helpdesk_resolve_priority rp"
                    . " LEFT JOIN helpdesk_cus_company c on(rp.cus_comp_id=c.cus_company_id)"
                    . " LEFT JOIN helpdesk_tr_prd_tier trp on(rp.tr_prd_tier_id=trp.tr_prd_tier_id)"
                    . " LEFT JOIN helpdesk_tr_opr_tier tro on(rp.tr_opr_tier_id=tro.tr_opr_tier_id)"
                    . " LEFT JOIN helpdesk_prd_tier p1 on(trp.prd_tier_lv1_id=p1.prd_tier_id)"
        ...
        public function getById($objective){
            ...
            $condition = "rp.id_resolve_priority = '$objective'";
            ...
            $table = "helpdesk_resolve rs";
            $field = "priority_id";
            $condition = "rs.cus_comp_id = '$s_cus_comp_id' and rs.tr_prd_tier_id = '$s_tr_prd_tier_id' and rs.tr_opr_tier_id = '$s_tr_opr_tier_id' and rs.id_resolve_priority = '$objective' group by rs.priority_id";
...
//            $table = " helpdesk_resolve rp";           
//            $condition = "id_resolve = '$objective'";
//            $result = $this->db->select($field, $table, $condition);
//            $rows = $this->db->num_rows($result);
//
//            if ($rows == 0) return null;
...
//            $s_tr_opr_tier_id = $objective_re["tr_opr_tier_id"];
//            
//            $sql_deleted = "delete from helpdesk_resolve where cus_comp_id = '{$s_cus_comp_id}' 
//                and tr_prd_tier_id = '{$s_tr_prd_tier_id}' and tr_opr_tier_id = '{$s_tr_opr_tier_id}'";
//            
//            return $result_de = $this->db->query($sql_deleted);
            
            $sql_deleted01 = "delete from helpdesk_resolve_priority where id_resolve_priority = '{$objective}'";
            $result_de01 = $this->db->query($sql_deleted01);
            
            $sql_deleted = "delete from helpdesk_resolve where id_resolve_priority = '{$objective}'";

            return $result_de = $this->db->query($sql_deleted);
        }
        
        public function restore_master($objective){
            return $this->deleteWithUpdate("helpdesk_resolve", "status", "A", "id_resolve = '$objective'");
        }
        
        public function insert(&$objective){
            $table = "helpdesk_resolve_priority";
            $field = "cus_comp_id, priority_id, tr_prd_tier_id, tr_opr_tier_id, created_by, created_date, modified_by, modified_date, resolve_sla";
            $data = "'{$objective["cus_comp_id"]}', '{$objective["sum_priority"]}', '{$objective["tr_prd_tier_id"]}', '{$objective["tr_opr_tier_id"]}',
                ...
                '{$objective["modified_date"]}', '{$objective["resolve_sla"]}'";

            $result = $this->db->insert($table, $field, $data);
            $objective["id_resolve_priority"] = $this->db->insert_id();
            ...
            for($i=0;$i<count($objective["chk_priority"]);$i++)
{
	if(trim($objective["chk_priority"][$i]) != "")
	{
		$table = "helpdesk_resolve";
            $field = "cus_comp_id, tr_prd_tier_id, tr_opr_tier_id, resolve_sla, status, created_by, created_date, modified_by, modified_date, priority_id, sum_resolve_sla, id_resolve_priority";
            $data = "'{$objective["cus_comp_id"]}', '{$objective["tr_prd_tier_id"]}', '{$objective["tr_opr_tier_id"]}', '{$objective["resolve_sla"]}',
                '{$objective["status"]}', '{$objective["created_by"]}', '{$objective["created_date"]}', {$objective["modified_by"]},
                '{$objective["modified_date"]}', '{$objective["chk_priority"][$i]}', '{$objective["sum_resolve_sla"]}', '{$objective["id_resolve_priority"]}'";
            ...
            if ($result){
                $objective["id_resolve_priority"] = $objective["id_resolve_priority"];
            }
...
//            $field = "cus_comp_id,tr_prd_tier_id,tr_opr_tier_id";
//            $table = " helpdesk_resolve rp";           
//            $condition = "id_resolve = '{$objective["id_resolve"]}'";
//            $result = $this->db->select($field, $table, $condition);
//            $rows = $this->db->num_rows($result);
//
//            if ($rows == 0) return null;
...
//
//            return $result_de = $this->db->query($sql_deleted);
            $sql_deleted01 = "delete from helpdesk_resolve_priority where id_resolve_priority = '{$objective["id_resolve_priority"]}'";
            $result_de01 = $this->db->query($sql_deleted01);
            
            $sql_deleted = "delete from helpdesk_resolve where id_resolve_priority = '{$objective["id_resolve_priority"]}'";
