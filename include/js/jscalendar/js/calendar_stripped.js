...
Calendar.tableMouseOver=function(ev){
    ...
    if(el.navtype==50&&target!=el){
        ...
        for(var i=range.length;--i>=0;)if(range[i]==current)break;while(count-->0)if(decrease){
...
Calendar.cellClick=function(el,ev){
    ...
    switch(el.navtype){
        ...
        case 50:
            ...
            for(var i=range.length;--i>=0;)if(range[i]==current)break;if(ev&&ev.shiftKey){
            if(--i<0)i=range.length-1;
        }else if(++i>=range.length)i=0;
            var newval=range[i];
            el.innerHTML=newval;
...
Calendar._keyEvent=function(ev){
    ...
    case 37:case 38:case 39:case 40:
        ...
        if(ne){
            ...
            else if(prev)prevMonth();else nextMonth();
        }
    }
    break;
case 13:
