    ...
    public static function addCard(Request $request, $card, $login)
    {
        if ($login) {
            ...
            foreach ($leave_status as $status => $label) {
                ...
                \Index\Home\Controller::renderCard($card, $icons[0], '{LNG_My leave}', number_format($value), '{LNG_Request for leave} '.$label, $url.$status);
            }
            ...
            if (isset($datas[0][0])) {
                ...
                \Index\Home\Controller::renderCard($card, $icons[1], '{LNG_Can be approve}', number_format($datas[0][0]), '{LNG_Request for leave} '.$leave_status[0], $url);
            }
            // รายการลาผู้อนุมัติรออนุมัติยกเลิก
            if (isset($datas[3][3])) {
                $url = WEB_URL.'index.php?module=eleave-report&amp;from='.$fiscalyear['from'].'&amp;to='.$fiscalyear['to'].'&amp;status=3';
                if ($login['status'] != 1) {
                    $url .= '&amp;type='.$login['id'];
                }
                \Index\Home\Controller::renderCard($card, $icons[2], '{LNG_Can be approve}', number_format($datas[3][3]), '{LNG_Request for leave} '.$leave_status[3], $url);
            }
        }
    }
}
