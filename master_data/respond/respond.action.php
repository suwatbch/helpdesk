<?php
    ...
    function unspecified() {
        ...
        $p = new helpdesk_respond($db);
         if(strUtil::isNotEmpty($_REQUEST["id_respond_priority"])){
            $objective = $_REQUEST["id_respond_priority"];
            $copy = 0;
        }else if(strUtil::isNotEmpty($_REQUEST["c_id_respond_priority"])){
            $objective = $_REQUEST["c_id_respond_priority"];
            $copy = 1;
        }
    ...
    function save(){
        global $db, $objective;
        $sum_respond_sla = strUtil::sum_sla($_REQUEST["respond_sla"]);

        # get input
        $objective = array(
            "id_respond_priority" => $_REQUEST["id_respond_priority"]
            , "cus_comp_id" => $_REQUEST["cus_comp_id"]
            , "tr_prd_tier_id" => $_REQUEST["tr_prd_tier_id"]
            , "tr_opr_tier_id" => $_REQUEST["tr_opr_tier_id"]
            , "respond_sla" => $_REQUEST["respond_sla"]
            , "sum_respond_sla" => $sum_respond_sla
            ...
            if (strUtil::isEmpty($objective["id_respond_priority"])){
            ...
            if ($result){
                $objective = $p->getById($objective["id_respond_priority"]);
