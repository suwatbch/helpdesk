<?php
   include_once "respond.action.php";
   include_once "../../include/function.php";
?>
<script type="text/javascript">
    function validate_sla(){
        if($("#cus_comp_id").val() == ""){
            ...
            return false;   
        }else if($("#respond_sla").val() == ""){
            jAlert('error', 'Please input Respond SLA (Hour)', 'Helpdesk System : Messages');
            $("#respond_sla").focus();
            return false;   
    ...
    function validate_de_respond(){
        ...
        $.ajax({
                ...
                data: "action=respond&cus_comp_id=" + $("#cus_comp_id").val() + "&tr_prd_tier_id=" + $("#tr_prd_tier_id").val() 
                    + "&tr_opr_tier_id=" + $("#tr_opr_tier_id").val() + "&id_respond_priority=" + $("#id_respond_priority").val()+"&chk_priority="+chk_priority,
                success: function(respone){
                    //alert(respone);
                    //$("#show_sql").html(respone);
                    if(respone == 1){ 
                        
                        jAlert('error', 'Respond Service Level Agreement Duplicate !!', 'Helpdesk System : Messages');
                        return false;
                   }else{
                       //alert('save');
                        page_submit("index.php?action=respond.php", "save")
                      }
                }
            });
    ...
    function back_master(){
        <?
            $_SESSION["current"] = "master_data/respond/index.php?action=respond_list.php";
    ...
    $(function(){
        ...
        $("#undo").click(function(){
            page_submit("index.php?action=respond.php&action_master=1");
        });

        $("#cancel").click(function(){
            page_submit("index.php?action=respond_list.php");
        });
    });
</script>
<script type="text/javascript">
...
<table width="80%" border="0" cellpadding="0" cellspacing="5" align="center">
        ...
        <td align="left">
            ...
            foreach ($objective["data_priority"] as $data_priority){
              ...
              if (count($s_list_priority) > 0){
                  if($objective["id_respond_priority"]!=""){
                       foreach ($s_list_priority as $list_prior){
                           if(in_array($list_prior["priority_id"], $data_pri)){
                               echo "<input type='checkbox' name='chk_priority[]' class='chk_priority' value='$list_prior[priority_id]' checked>&nbsp;$list_prior[priority_desc]"."&nbsp;&nbsp;";
                           }else{
                                echo "<input type='checkbox' name='chk_priority[]' class='chk_priority' value='$list_prior[priority_id]'>&nbsp;$list_prior[priority_desc]"."&nbsp;&nbsp;";
                            }
                    }
               }else{
                   foreach ($s_list_priority as $list_prior){
    ...
    <tr>
        <td class="tr_header">Respond SLA (Time)(HHHH:MM:SS)<span class="required">*</span></td>
        <td><input type="text" id="respond_sla" name="respond_sla" style="width: 100%;" value="<?=$objective["respond_sla"]?>" maxlength="10" rule="time" /></td>
    </tr>
<!--    <tr>
    ...
    <? if(strUtil::isNotEmpty($objective['id_respond_priority'])){ ?>
    <tr>
...
<input type="hidden" name="id_respond_priority" id="id_respond_priority" value="<? if($copy==1){ echo "";}else { echo $objective["id_respond_priority"]; }?>"/>
<input type="hidden" name="c_id_respond_priority" id="c_id_respond_priority" value="<? if($copy==1){ echo $objective["id_respond_priority"];}else { echo "" ; }?>"/>
