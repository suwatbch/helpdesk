<?php
   include_once "subgrp.action.php";
   include_once "../../include/function.php";
?>
<script type="text/javascript">
    function validate_subgrp_mas(){
        if($("#company_id").val() == ""){
            ...
            jAlert('error', 'Please input SubGroup Name', 'Helpdesk System : Messages');
            $("#org_name").focus();
            return false;   
        }else{
            return true;
    ...
    function validate_de_subgrp(){
        $.ajax({
                type: "GET",
                url: "../common/validate_detail.php",
                data: "action=subgrp_master&org_id=" + $("#s_org_id").val() + "&company_id=" + $("#company_id").val() + "&org_name=" + $("#org_name").val(),
                success: function(respone){
                    //alert(respone);
                    if(respone == 1){ 
                        jAlert('error', 'Sub Group Master Duplicate !!', 'Helpdesk System : Messages');
                        return false;
                   }else{
                        page_submit("index.php?action=subgrp.php", "save")
                      }
                }
            });
    ...
    function back_master(){
        <?
            $_SESSION["current"] = "master_data/subgroup_master/index.php?action=subgrp_list.php";
    ...
    $(function(){
        $("#save").click(function(){
            if (validate_subgrp_mas()){
                validate_de_subgrp();
            }
        });

        $("#undo").click(function(){
            page_submit("index.php?action=subgrp.php&action_master=1");
        });

        $("#cancel").click(function(){
            page_submit("index.php?action=subgrp_list.php");
        });
    });
</script>
<script type="text/javascript">
...
<table width="80%" border="0" cellpadding="0" cellspacing="5" align="center">
    ...
    <tr>
        <td class="tr_header">Sub Group Name<span class="required">*</span></td>
...
<input type="hidden" name="s_org_grp" id="s_org_grp" value="<?=$objective["s_group_id"]?>"/>