<?php
    ...
    function search_unrelease($cus_comp_id="",$prd_tier_id1="",$prd_tier_id2="",$prd_tier_id3=""){
        ...
        $f_objective = $p->search_unrelease($cratiria, $page);
    ...
    function save(){
        global $db, $unrelease;
        $unrelease = array(
         "km_id" => split(",",$_REQUEST["select_release"])
            ...
            $p = new helpdesk_identify_km($db);
            if (strUtil::isNotEmpty($unrelease["km_id"])){
                if($_REQUEST["mode"] == 1){
                    $result = $p->update_unrelease($unrelease);
                }else if($_REQUEST["mode"]==2){
                   $result = $p->update_identify($unrelease); 
                }
            } 
            $db->end_transaction($result);
